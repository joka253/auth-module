...
export default class Oauth2Scheme {
  ...
  async login ({ params, state, nonce } = {}) {
    const opts = {
      ...
      state: state || nanoid(),
      code_challenge_method: this.options.code_challenge_method || 'implicit',
      ...params
    }
    ...
    if (this.options.response_mode) {
      opts.response_mode = this.options.response_mode
    }
    if (this.options.acr_values) {
      opts.acr_values = this.options.acr_values
    }
    ...
    if (this.options.response_type === 'code' && parsedQuery.code) {
      const data = await this.$auth.request({
        ...
        data: encodeQuery({
          ...
          code_verifier: (this.options.code_challenge_method && this.options.code_challenge_method !== 'implicit' ? localStorage.getItem('pkce_code_verifier') : undefined)
        })
      })
